...
trait WishlistControls {
    ...
    public function render_wishlist_controls() {
        do_action( 'wishglut_before_render_wishlist_controls' );
        
        $output = $this->internal_render_wishlist_controls();
        
        do_action( 'wishglut_after_render_wishlist_controls' );
        
        return $output;
    }
    
    public static function trigger_wishlist_controls_render() {
        do_action( 'wishglut_render_wishlist_controls' );
    }
...
// phpcs:ignore WordPress.Security.NonceVerification.Recommended
        ...
        ob_start();
        ?>
        <div class="wishglut-wishlist-controls">
            <div class="wishlist-controls-row">
                <!-- Sorting Enhancements -->
                <div class="wishlist-sort">
                    <label for="wishlist-sort"><?php echo esc_html__('Sort by:', 'wishglut'); ?></label>
                    <select class="wishlist-sort-select">
                        <option value="date_added" <?php selected($current_sort, 'date_added'); ?>><?php echo esc_html__('Date Added', 'wishglut'); ?></option>
                        <option value="name" <?php selected($current_sort, 'name'); ?>><?php echo esc_html__('Product Name', 'wishglut'); ?></option>
                        <option value="price_low" <?php selected($current_sort, 'price_low'); ?>><?php echo esc_html__('Price: Low to High', 'wishglut'); ?></option>
                        <option value="price_high" <?php selected($current_sort, 'price_high'); ?>><?php echo esc_html__('Price: High to Low', 'wishglut'); ?></option>
                        <option value="availability" <?php selected($current_sort, 'availability'); ?>><?php echo esc_html__('Availability', 'wishglut'); ?></option>
                    </select>
                </div>

                <!-- Filter Enhancements -->
                <div class="wishlist-filter">
                    <label for="wishlist-filter"><?php echo esc_html__('Filter:', 'wishglut'); ?></label>
                    <select class="wishlist-filter-select">
                        <option value="all" <?php selected($current_filter, 'all'); ?>><?php echo esc_html__('All Products', 'wishglut'); ?></option>
                        <option value="in_stock" <?php selected($current_filter, 'in_stock'); ?>><?php echo esc_html__('In Stock Only', 'wishglut'); ?></option>
                        <option value="out_stock" <?php selected($current_filter, 'out_stock'); ?>><?php echo esc_html__('Out of Stock', 'wishglut'); ?></option>
                        <option value="on_sale" <?php selected($current_filter, 'on_sale'); ?>><?php echo esc_html__('On Sale', 'wishglut'); ?></option>
                    </select>
                </div>
...
// phpcs:ignore WordPress.Security.NonceVerification.Recommended
    ...
    public function init_wishlist_controls_hooks() {
        add_action( 'wishglut_render_wishlist_controls', array( $this, 'handle_render_wishlist_controls' ), 10 );
    }

...
trait WishlistTechnicalImprovements {
    ...
    public function init_cache_compatibility() {
        ...
        add_action('wp_ajax_wishglut_toggle_wishlist', [$this, 'add_no_cache_headers'], 1);
        add_action('wp_ajax_nopriv_wishglut_toggle_wishlist', [$this, 'add_no_cache_headers'], 1);
    }
    
    public function add_wishlist_cookies_to_wp_rocket($cookies) {
        $wishlist_cookies = [
            'wishglutw_guest_user_id',
            'wishglut_wishlist_items',
            'wishglut_wishlist_count'
        ];
    ...
    public function wp_super_cache_compatibility() {
        $request_uri = isset($_SERVER['REQUEST_URI']) ? esc_url_raw(wp_unslash($_SERVER['REQUEST_URI'])) : '';
        if (isset($_COOKIE['wishglutw_guest_user_id']) || 
            strpos($request_uri, 'wishglut') !== false) {
    ...
    public function litespeed_cache_compatibility() {
        if (isset($_COOKIE['wishglutw_guest_user_id'])) {
            do_action('litespeed_vary_add', 'wishglut_wishlist');
        }
    }
    
    public function exclude_wishlist_js_from_autoptimize($exclude) {
        return $exclude . ', wishglut-wishlist, wishlist-qr';
    }
    
    public function add_cache_busting_scripts() {
        $cache_version = get_option('wishglut_wishlist_cache_version', time());
        wp_localize_script('wishglut-wishlist', 'wishglut_cache', [
            'version' => $cache_version,
            'nonce' => wp_create_nonce('wishglut_cache_nonce')
        ]);
    }
    ...
    public function init_gdpr_compliance() {
        ...
        add_action('wp_ajax_wishglut_gdpr_consent', [$this, 'handle_gdpr_consent']);
        add_action('wp_ajax_nopriv_wishglut_gdpr_consent', [$this, 'handle_gdpr_consent']);
    ...
    public function add_gdpr_consent_modal() {
        ...
        ?>
        <div id="wishglut-gdpr-modal" class="wishglut-gdpr-modal" style="display: none;">
            <div class="wishglut-gdpr-content">
                <h3><?php esc_html_e('Privacy & Cookies', 'wishglut'); ?></h3>
                <p><?php esc_html_e('We use cookies to remember your wishlist items and provide a better shopping experience. By continuing to use our wishlist feature, you consent to our use of cookies.', 'wishglut'); ?></p>
                <div class="wishglut-gdpr-actions">
                    <button id="wishglut-gdpr-accept" class="wishglut-gdpr-btn accept">
                        <?php esc_html_e('Accept', 'wishglut'); ?>
                    </button>
                    <button id="wishglut-gdpr-decline" class="wishglut-gdpr-btn decline">
                        <?php esc_html_e('Decline', 'wishglut'); ?>
                    </button>
                    <a href="<?php echo esc_url(get_privacy_policy_url()); ?>" target="_blank" class="wishglut-gdpr-policy">
                        <?php esc_html_e('Privacy Policy', 'wishglut'); ?>
                    </a>
                </div>
            </div>
        </div>
        ...
        .wishglut-gdpr-btn.accept {
            background: #4CAF50;
            color: white;
        }
        
        .wishglut-gdpr-btn.decline {
            background: #f44336;
            color: white;
        }
        
        .wishglut-gdpr-policy {
            color: #007cba;
    ...
    public function handle_gdpr_consent() {
        check_ajax_referer('wishglut_gdpr_nonce', 'nonce');
        
        if (!isset($_POST['consent'])) {
            wp_send_json_error(['message' => __('Consent parameter is required.', 'wishglut')]);
            return;
        }
        ...
        if ($consent === 'accept') {
            setcookie('wishglut_gdpr_consent', 'accepted', time() + (365 * 24 * 60 * 60), '/');
            wp_send_json_success(['message' => __('Consent accepted', 'wishglut')]);
        } else {
            setcookie('wishglut_gdpr_consent', 'declined', time() + (365 * 24 * 60 * 60), '/');
            // Clear wishlist cookies
            setcookie('wishglutw_guest_user_id', '', time() - 3600, '/');
            wp_send_json_success(['message' => __('Consent declined', 'wishglut')]);
        }
    }
    
    public function register_wishlist_data_exporter($exporters) {
        $exporters['wishglut-wishlist'] = [
            'exporter_friendly_name' => __('Wishglut Wishlist Data', 'wishglut'),
    ...
    public function register_wishlist_data_eraser($erasers) {
        $erasers['wishglut-wishlist'] = [
            'eraser_friendly_name' => __('Wishglut Wishlist Data', 'wishglut'),
    ...
    public function export_wishlist_data($email_address) {
        ...
        global $wpdb;
        $table = $wpdb->prefix . 'wishglut_wishlist';
        ...
        if ($wishlist_data) {
            $export_items[] = [
                'group_id' => 'wishglut-wishlist',
                'group_label' => __('Wishlist Data', 'wishglut'),
                'item_id' => 'wishlist-' . $user->ID,
                'data' => [
                    [
                        'name' => __('Product IDs', 'wishglut'),
                        'value' => $wishlist_data->product_ids
                    ],
                    [
                        'name' => __('Date Added', 'wishglut'),
                        'value' => $wishlist_data->product_added_time
                    ],
                    [
                        'name' => __('Notifications', 'wishglut'),
    ...
    public function erase_wishlist_data($email_address) {
        ...
        return [
            ...
            'items_retained' => 0,
            'messages' => $result ? [__('Wishlist data erased.', 'wishglut')] : [],
            'done' => true
        ];
    }
    
    private function gdpr_consent_required() {
        return !isset($_COOKIE['wishglut_gdpr_consent']) && 
               !is_user_logged_in() && 
    ...
    public function init_rtl_support() {
        ...
        add_filter('wishglut_wishlist_css_class', [$this, 'add_rtl_class']);
    }
    
    public function enqueue_rtl_styles() {
        if (is_rtl()) {
            wp_enqueue_style(
                'wishglut-wishlist-rtl',
    ...
    public function init_performance_optimizations() {
        ...
        add_filter('wishglut_wishlist_ajax_response', [$this, 'optimize_ajax_response']);
    }
    ...
    public function init_object_caching() {
        ...
        add_filter('wishglut_wishlist_count', [$this, 'cache_wishlist_count'], 10, 2);
        
        // Cache product data
        add_filter('wishglut_wishlist_product_data', [$this, 'cache_product_data'], 10, 2);
    }
    
    public function cache_wishlist_count($count, $user_id) {
        $cache_key = 'wishglut_wishlist_count_' . $user_id;
        $cached_count = wp_cache_get($cache_key, 'wishglut_wishlist');
        
        if ($cached_count === false) {
            wp_cache_set($cache_key, $count, 'wishglut_wishlist', 300); // 5 minutes
            return $count;
        }
    ...
    public function add_async_defer_attributes($tag, $handle) {
        if (strpos($handle, 'wishglut-wishlist') !== false) {
    ...
    public function optimize_fid() {
        ...
        wp_script_add_data('wishglut-wishlist', 'defer', true);
        
        // Use passive event listeners
        wp_add_inline_script('wishglut-wishlist', '
            document.addEventListener("DOMContentLoaded", function() {
                const buttons = document.querySelectorAll(".wishglut-wishlist-btn");
                buttons.forEach(btn => {
    ...
    public function optimize_cls() {
        ...
        ?>
        <style>
        .wishglut-wishlist-container {
            min-height: 200px;
        }
        
        .wishglut-wishlist-table {
            width: 100%;
            table-layout: fixed;
        }
        
        .wishglut-wishlist-button {
            width: 120px;
            height: 40px;
        }
        
        .wishglut-wishlist-loading {
            width: 20px;
            height: 20px;
        }
        </style>
    ...
    public function init_lazy_loading() {
        ...
        add_filter('wishglut_wishlist_product_image', [$this, 'add_image_placeholder'], 10, 2);
    }
    ...
    public function add_lazy_loading_attributes($attr, $attachment, $size) {
        if ($this->is_wishlist_context()) {
            ...
            $attr['class'] = ($attr['class'] ?? '') . ' wishglut-lazy-image';
        }
        return $attr;
    }
    ...
    public function add_lazy_loading_script() {
        ...
        document.addEventListener('DOMContentLoaded', function() {
            const lazyImages = document.querySelectorAll('.wishglut-lazy-image');
                ...
                const imageObserver = new IntersectionObserver((entries, observer) => {
                    entries.forEach(entry => {
                        if (entry.isIntersecting) {
                            ...
                            img.classList.remove('wishglut-lazy-image');
                            img.classList.add('wishglut-lazy-loaded');
        ...
        .wishglut-lazy-loaded {
            opacity: 1;
        }
        
        .wishglut-lazy-image::before {
            content: '';
            display: block;
            width: 100%;
            height: 100%;
    ...
    private function is_wishlist_context() {
        return $this->is_wishlist_page() || 
               (isset($_SERVER['REQUEST_URI']) && strpos(esc_url_raw(wp_unslash($_SERVER['REQUEST_URI'])), 'wishglut') !== false) ||
               // phpcs:ignore WordPress.Security.NonceVerification.Missing
               (defined('DOING_AJAX') && DOING_AJAX && isset($_POST['action']) && strpos(sanitize_text_field(wp_unslash($_POST['action'])), 'wishglut') !== false);
    }

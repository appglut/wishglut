...
trait WishlistButtons {
	...
	public function wishglut_add_wishlist_button_single() {
	...
	$existing_entry = $wpdb->get_var( $wpdb->prepare(
		...
		if ( $existing_entry > 0 ) {
			...
			$button_text = $this->enhancements['wishlist-product-button-text-after-added'] ?? __( 'Added to Wishlist', 'wishglut' );
			...
			$button_text = $this->enhancements['wishlist-product-button-text'] ?? __( 'Add to Wishlist', 'wishglut' );
		...
		do_action('wishglut_wishlist_pro_features_loaded', $product_id, 'single-product');
	...
	public function wishglut_add_wishlist_button_shop() {
		global $wpdb, $product, $wishglut_rendering_shop_layout;
	...
	$existing_entry = $wpdb->get_var( $wpdb->prepare(
		...
		if ( $existing_entry > 0 ) {
			...
			$button_text = $this->enhancements['wishlist-shop-button-text-after-added'] ?? __( 'Added To Wishlist', 'wishglut' );
			...
			$button_text = $this->enhancements['wishlist-shop-button-text'] ?? __( 'Add To Wishlist', 'wishglut' );
			$button_class = "not-shopgw-added";
			$href = '#';
		}
		...
		echo "<div class='wishglut_wishlist_container'>";
		echo "<div class='wishglut_wishlist shop-page'>";
	...
	public function wishglut_add_wishlist_button_category() {
		global $wpdb, $product, $wishglut_rendering_shop_layout;
	...
	$existing_entry = $wpdb->get_var( $wpdb->prepare(
		...
		if ( $existing_entry > 0 ) {
			...
			$button_text = $this->enhancements['wishlist-archive-button-text-after-added'] ?? __( 'Added To Wishlist', 'wishglut' );
			...
			$button_text = $this->enhancements['wishlist-archive-button-text'] ?? __( 'Add To Wishlist', 'wishglut' );
			$button_class = "not-shopgw-added";
			$href = '#';
		}
		...
		echo "<div class='wishglut_wishlist_container'>";
		echo "<div class='wishglut_wishlist archive-page'>";
		...
		if ( $this->enhancements['wishlist-archive-enable-movelist'] === '1' && is_user_logged_in() ) {
			$move_button_text = __( "Move to List", 'wishglut' );
			echo "<div class='wishglut_wishlist_movelist archive-page'>";
			echo '<a href="#" class="button move_to_list" data-product-id="' . esc_attr( $product_id ) . '"><span class="button-text">' . esc_html( $move_button_text ) . '</span></a>';
			echo "</div>";
		}

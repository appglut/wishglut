...
trait WishlistMerge {
    ...
    public function wishglut_set_merge_wishlist_transient( $user_login, $user ) {
		// Transient removed - no longer needed
	}

	public function wishglut_merge_guest_wishlist() {
...
// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
				$guest_wishlist = $wpdb->get_var( $wpdb->prepare(
					"SELECT product_ids FROM {$wpdb->prefix}wishglut_wishlist WHERE wish_user_id = %s", $guest_id
...
// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
							$existing_entry = $wpdb->get_var( $wpdb->prepare(
								"SELECT product_ids FROM {$wpdb->prefix}wishglut_wishlist WHERE wish_user_id = %d", $user_id
							) ); // phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching -- Custom table query
...
// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
    ...
    private function get_wishglutw_guest_user_id() {
		if ( isset( $_COOKIE['wishglutw_guest_user_id'] ) ) {
			return sanitize_text_field( wp_unslash($_COOKIE['wishglutw_guest_user_id']) );
		}
		return ''; // Return an empty string or handle as needed if the cookie is not set
	}

	public function wishglut_cleanup_old_guest_products() {
		global $wpdb;
...
// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
		$guest_entries = $wpdb->get_results($wpdb->prepare("
			...
			FROM {$wpdb->prefix}wishglut_wishlist 
...
// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
		$old_entries_deleted = $wpdb->query($wpdb->prepare("
			DELETE FROM {$wpdb->prefix}wishglut_wishlist 
			WHERE wish_user_id LIKE %s 
	...
	public function wishglut_schedule_guest_cleanup() {
		if (!wp_next_scheduled('wishglut_daily_guest_cleanup')) {
			wp_schedule_event(time(), 'daily', 'wishglut_daily_guest_cleanup');
		}
	}

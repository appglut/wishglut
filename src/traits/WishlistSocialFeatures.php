...
trait WishlistSocialFeatures {
    ...
    public function render_social_share_buttons($list_name = '') {
        // Get enhancements
        $enhancements = get_option( 'agwishglut_wishlist_options' );
        ...
        // Get current user ID
        $user_id = is_user_logged_in() ? get_current_user_id() : $this->get_wishglutw_guest_user_id();
        
        if (!$user_id) {
            return '<!-- No user ID found for sharing -->';
        }

        $share_title = isset($enhancements['social-share-title']) ? esc_html($enhancements['social-share-title']) : esc_html__('Share Wishlist:', 'wishglut');
        
        // Generate shareable URL with encrypted data
        $current_url = $this->get_social_shareable_wishlist_url($user_id, $list_name);
        
        $html = '<div class="wishglut-social-share">';
        $html .= '<span class="share-title">' . $share_title . '</span>';

        // Facebook Share
        if (!empty($enhancements['enable-facebook-share'])) {
            $html .= '<a href="https://www.facebook.com/sharer/sharer.php?u=' . urlencode($current_url) . '" target="_blank" class="facebook-share social-share-btn" title="' . esc_attr__('Share on Facebook', 'wishglut') . '"><i class="fab fa-facebook"></i></a>';
        }

        // Dynamic share text based on list name
        // translators: %s is the wishlist name
        $list_text = $list_name ? sprintf(esc_html__('Check out my "%s" wishlist!', 'wishglut'), $list_name) : esc_html__("Check out my wishlist!", 'wishglut');
        // translators: %s is the wishlist name
        $list_title = $list_name ? sprintf(esc_html__('My "%s" Wishlist', 'wishglut'), $list_name) : esc_html__("My Wishlist", 'wishglut');
        ...
        if (!empty($enhancements['enable-twitter-share'])) {
            ...
            $html .= '<a href="https://twitter.com/intent/tweet?url=' . urlencode($current_url) . '&text=' . urlencode($twitter_text) . '" target="_blank" class="twitter-share social-share-btn" title="' . esc_attr__('Share on Twitter', 'wishglut') . '"><i class="fab fa-x-twitter"></i></a>';
        }
        ...
        if (!empty($enhancements['enable-whatsapp-share'])) {
            ...
            $html .= '<a href="https://api.whatsapp.com/send?text=' . urlencode($whatsapp_text . " " . $current_url) . '" target="_blank" class="whatsapp-share social-share-btn" title="' . esc_attr__('Share on WhatsApp', 'wishglut') . '"><i class="fab fa-whatsapp"></i></a>';
        }

        // Pinterest Share
        if (!empty($enhancements['enable-pinterest-share'])) {
            $pinterest_description = $list_text;
            $html .= '<a href="https://pinterest.com/pin/create/button/?url=' . urlencode($current_url) . '&description=' . urlencode($pinterest_description) . '" target="_blank" class="pinterest-share social-share-btn" title="' . esc_attr__('Share on Pinterest', 'wishglut') . '"><i class="fab fa-pinterest"></i></a>';
        }
        ...
        if (!empty($enhancements['enable-linkedin-share'])) {
            ...
            $html .= '<a href="https://www.linkedin.com/shareArticle?mini=true&url=' . urlencode($current_url) . '&title=' . urlencode($linkedin_title) . '&summary=' . urlencode($list_text) . '" target="_blank" class="linkedin-share social-share-btn" title="' . esc_attr__('Share on LinkedIn', 'wishglut') . '"><i class="fab fa-linkedin"></i></a>';
        }
        ...
        if (!empty($enhancements['enable-telegram-share'])) {
            ...
            $html .= '<a href="https://telegram.me/share/url?url=' . urlencode($current_url) . '&text=' . urlencode($telegram_text) . '" target="_blank" class="telegram-share social-share-btn" title="' . esc_attr__('Share on Telegram', 'wishglut') . '"><i class="fab fa-telegram"></i></a>';
        }
        ...
        if (!empty($enhancements['enable-email-share'])) {
            ...
            $html .= '<a href="mailto:?subject=' . urlencode($email_subject) . '&body=' . urlencode($email_body . $current_url) . '" class="email-share social-share-btn" title="' . esc_attr__('Share via Email', 'wishglut') . '"><i class="fas fa-envelope"></i></a>';
        }
...
// phpcs:ignore WordPress.Security.NonceVerification.Recommended
        ...
        if (!$encrypted_data) {
            echo '<div class="wishglut-error">' . esc_html__('Invalid share link.', 'wishglut') . '</div>';
            return true;
        }
        ...
        if (!$share_data || !isset($share_data['user_id'])) {
            echo '<div class="wishglut-error">' . esc_html__('Invalid or corrupted share link.', 'wishglut') . '</div>';
            return true;
        }
        ...
        if (isset($share_data['timestamp'])) {
            ...
            if ($days_old > 30) {
                echo '<div class="wishglut-warning">' . esc_html__('This share link is quite old and may not reflect the current wishlist.', 'wishglut') . '</div>';
            }
        }
        ...
        echo '<div class="wishglut-shared-wishlist">';
        echo '<div class="shared-header">';
        echo '<h3><i class="fas fa-share-alt"></i> ' . esc_html__('Shared Wishlist', 'wishglut') . '</h3>';
        
        if ($list_name) {
            // translators: %s is the wishlist name
            echo '<p class="list-name">' . sprintf(esc_html__('List: %s', 'wishglut'), '<strong>' . esc_html($list_name) . '</strong>') . '</p>';
        }
        
        if ($source === 'social_share') {
            echo '<p class="share-source">' . esc_html__('Shared via social media', 'wishglut') . '</p>';
        }
        echo '</div>';
        ...
        if (isset($wishlist_data['sublists']) && !empty($wishlist_data['sublists'])) {
            ...
            if ($wishlist_data && !empty($wishlist_data['product_ids'])) {
                ...
                echo '<p>' . esc_html__('This wishlist is empty or no longer available.', 'wishglut') . '</p>';
                echo '</div>';
            }
        }
    ...
    private function get_social_shared_wishlist_data($user_id, $list_name = '') {
        ...
        $sublists = apply_filters('wishglut_get_user_sublists', [], $user_id);
    ...
    private function render_social_wishlist_tabs($wishlist_data, $active_list = '') {
        ...
        echo esc_html__('Main List', 'wishglut');
        ...
        if (!empty($wishlist_data['product_ids'])) {
            ...
            echo '<div class="empty-wishlist">';
            echo '<i class="fas fa-heart-broken"></i>';
            echo '<p>' . esc_html__('The main wishlist is empty.', 'wishglut') . '</p>';
            echo '</div>';
        }
        echo '</div>';
    ...
    private function render_shared_social_wishlist_table($wishlist_data, $list_name = '') {
        ...
        if (empty($product_ids)) {
            echo '<div class="empty-wishlist">';
            echo '<i class="fas fa-heart-broken"></i>';
            echo '<p>' . esc_html__('This wishlist is empty.', 'wishglut') . '</p>';
            echo '</div>';
            return;
        }
        ...
        echo '<span class="item-count"><i class="fas fa-list"></i> ' . sprintf(esc_html__('%d items', 'wishglut'), count($product_ids)) . '</span>';
        if ($list_name) {
    ...
    private function render_shared_social_product_card($product) {
        ...
        if ($on_sale) {
            echo '<span class="sale-badge">' . esc_html__('SALE', 'wishglut') . '</span>';
        }
        echo '</a>';
        echo '</div>';
        ...
        if ($is_in_stock) {
            ...
            if ($stock_quantity && $stock_quantity <= 10) {
                echo '<span class="low-stock"><i class="fas fa-exclamation-triangle"></i> ' . 
                     ...
                     sprintf(esc_html__('Only %d left!', 'wishglut'), esc_html($stock_quantity)) . '</span>';
            } else {
                echo '<span class="in-stock"><i class="fas fa-check"></i> ' . esc_html__('In Stock', 'wishglut') . '</span>';
            }
        } else {
            echo '<span class="out-of-stock"><i class="fas fa-times"></i> ' . esc_html__('Out of Stock', 'wishglut') . '</span>';
        }
        echo '</div>';
        ...
        echo '<i class="fas fa-eye"></i> ' . esc_html__('View Product', 'wishglut');
        echo '</a>';
        echo '</div>';
    ...
    public function generate_social_share_styles() {
        ...
        $styles .= '.wishglut-social-share .share-title {';
        ...
        $styles .= '.wishglut-social-share .social-share-btn {';
        ...
        $styles .= '.wishglut-social-share .social-share-btn:hover {';
        $styles .= 'opacity: ' . $hover_opacity . ';';
        $styles .= '}';

        $styles .= '.wishglut-social-share .social-share-btn i {';
        ...
        foreach ($platform_colors as $platform => $color) {
            $styles .= '.wishglut-social-share .' . $platform . '-share {';
    ...
    public function init_wishlist_social_features_hooks() {
        add_action( 'wishglut_render_social_share_buttons', array( $this, 'handle_render_social_share_buttons' ), 10, 1 );
        add_action( 'wishglut_render_wishlist_popular', array( $this, 'handle_render_wishlist_popular' ), 10, 1 );
    }

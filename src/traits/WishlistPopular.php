...
trait WishlistPopular {
    ...
    private function render_wishlist_popular($product_ids) {
      ...
      <h4> <?php echo esc_html__('Others also wishlisted:', 'wishglut')  ?></h4>
    ...
    private function get_popular_wishlist_items($current_product_ids) {
        ...
        if (empty($all_wishlists)) {
            return '<p class="no-popular-items">' . esc_html__('No popular items found.', 'wishglut') . '</p>';
        }
        ...
        if (empty($product_counts)) {
            return '<p class="no-popular-items">' . esc_html__('No recommendations available.', 'wishglut') . '</p>';
        }
    ...
    private function render_popular_items_grid($popular_product_ids, $product_counts) {
        ...
        if (count($product_counts) > 5) {
            ...
            $output .= __('View All Popular Products', 'wishglut') . ' →';
            $output .= '</a>';
            $output .= '</div>';
        }
        ...
        return $items_found > 0 ? $output : '<p class="no-popular-items">' . esc_html__('No popular items available at the moment.', 'wishglut') . '</p>';
    }
    ...
    private function render_single_popular_item($product, $product_id, $wishlist_count) {
        ...
        $output .= '<span class="wishlist-count">👥 ' . 
            ...
            sprintf(__('%d wishlisted', 'wishglut'), $wishlist_count) . '</span>';
        $output .= '</div>';
    ...
    private function get_wishlist_button_data($is_in_wishlist, $product_id) {
        ...
        if ($is_in_wishlist) {
            $wishlist_button_text = $this->enhancements['wishlist-product-button-text-after-added'] ?? __('Added to Wishlist', 'wishglut');
            ...
            $wishlist_button_text = $this->enhancements['wishlist-product-button-text'] ?? __('Add to Wishlist', 'wishglut');
        ...
        if ( $this->enhancements['wishlist-require-login'] == true && ! is_user_logged_in() ) {
            ...
            $wishlist_button_text = __( 'Login Required', 'wishglut' );
        ...
        if ($product->is_in_stock()) {
            ...
            if ($stock_quantity && $stock_quantity <= 10) {
                return '<div class="stock-status low-stock">⚠️ ' . 
                    ...
                    sprintf(__('Only %d left!', 'wishglut'), $stock_quantity) . '</div>';
            } else {
                return '<div class="stock-status in-stock">✅ ' . esc_html__('In Stock', 'wishglut') . '</div>';
            }
        } else {
            return '<div class="stock-status out-of-stock">❌ ' . esc_html__('Out of Stock', 'wishglut') . '</div>';
        }
    }

    private function render_popular_item_wishlist_button($product_id, $button_data) {
        $output = '<div class="wishglut_wishlist single-product popular-wishlist-container">';

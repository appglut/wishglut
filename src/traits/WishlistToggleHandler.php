...
trait WishlistToggleHandler {
    ...
    public function wishglut_toggle_wishlist_callback() {
        global $wpdb;

        // Verify nonce for security
        if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nonce'] ) ), 'wishglut_wishlist_nonce' ) ) {
    ...
    private function remove_product_for_guest($guest_id, $product_id, $table_name, $button_config, $second_click_action, $wishlist_page_url, $checkout_page_url) {
        ...
        if ( $existing_entry ) {
            ...
            wp_send_json_error( [ 'message' => __( 'No product found for removal.', 'wishglut' ) ] );
            exit;
        }
    }
    ...
    private function build_remove_response($button_config, $second_click_action, $wishlist_page_url, $checkout_page_url, $product_id) {
        ...
        switch ( $second_click_action ) {
            case 'goto-wishlist':
                ...
                $notification_text = $this->enhancements['wishlist-product-removed-notification-text'] ?? __('product removed from wishlist', 'wishglut');
                $response_data['notification_text'] = $this->wishglut_parse_notification_text($notification_text, $product_id);
            ...
            case 'show-already-exist':
                ...
                $response_data['notification_text'] = __( 'Product already added to the wishlist', 'wishglut' );
    ...
    private function build_add_response($button_config, $second_click_action, $wishlist_page_url, $checkout_page_url, $product_id) {
        ...
        switch ( $second_click_action ) {
            case 'goto-wishlist':
                ...
                $response_data['href'] = $wishlist_page_url;
                $notification_text = $this->enhancements['wishlist-product-added-notification-text'] ?? __('Product added to wishlist', 'wishglut');
                $response_data['notification_text'] = $this->wishglut_parse_notification_text($notification_text, $product_id);
                $response_data['perform_toggle'] = false;
                break;
            case 'show-already-exist':

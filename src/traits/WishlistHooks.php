...
trait WishlistHooks {
    ...
    public function wishglut_register_wishlist_buttons() {
		...
		add_action( 'wp_footer', [ $this, 'wishglut_add_notification_container' ] );
		...
		if ( ! empty( $this->enhancements['wishlist-enable-product-page'] ) ) {
			...
			add_action( $hook, [ $this, 'wishglut_add_wishlist_button_single' ], 15 );
		}
		...
		if ( ! empty( $this->enhancements['wishlist-enable-shop-page'] ) ) {
			...
			add_action( $hook, [ $this, 'wishglut_add_wishlist_button_shop' ], 15 );
		}
		...
		if ( ! empty( $this->enhancements['wishlist-enable-archive-page'] ) ) {
			...
			add_action( $hook, [ $this, 'wishglut_add_wishlist_button_category' ], 20 );
		}
	}
     ...
     */
    public function wishglut_add_notification_container() {
        ...
        if ( is_product() || is_shop() || is_product_category() || is_product_tag() || is_product_taxonomy() ) {
            echo '<div id="wishglut-wishlist-notification" style="display: none;"></div>';
        }
    }

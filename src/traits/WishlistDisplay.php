...
trait WishlistDisplay {
    ...
    public function wishglut_load_wishlist_content() {
        check_ajax_referer( 'shopLayouts_nonce', 'nonce' );

        $content = $this->wishglut_wishlist_shortcode();
    ...
    public function wishglut_wishlist_shortcode() {
        global $wpdb;
        ...
        $wishlist_sublist = apply_filters('wishglut_get_user_sublists', [], $user_id);
        
        ob_start();

        echo "<div class='wishglut-wishlist-container'>";

        // Check for multilist functionality
        if (function_exists('is_plugin_active') && 
            is_plugin_active('wishglut-wishlist-pro/wishglut-wishlist-pro.php') && 
        ...
        echo "<div id='wishglut-wishlist-notification'></div>";
    ...
    private function render_single_wishlist($wishlist_data) {
        ...
        echo '<div class="shoglut-wishlist-tabs" id="wishglut-normal-wishlist">';
        ...
        do_action( 'wishglut_before_wishlist_content', $wishlist_data, $product_ids_filter );

        // Render action buttons
        $this->render_wishlist_action_buttons($product_ids_filter, 'main', '');
        
        // Hook: After action buttons
        do_action( 'wishglut_after_action_buttons', $wishlist_data, $product_ids_filter );
        ...
        do_action( 'wishglut_before_wishlist_table', $wishlist_data, $product_ids_filter );

        // Render table
        $this->render_wishlist_table( $product_ids_single, 'main' );

        // Hook: After wishlist table
        do_action( 'wishglut_after_wishlist_table', $wishlist_data, $product_ids_filter );

        
        // Hook: Before popular products
        do_action( 'wishglut_before_popular_products', $wishlist_data, $product_ids_filter );

        // Hook: After popular products
        do_action( 'wishglut_after_popular_products', $wishlist_data, $product_ids_filter );

        // Hook: After wishlist content
        do_action( 'wishglut_after_wishlist_content', $wishlist_data, $product_ids_filter );
    ...
    private function render_multilist_wishlist($wishlist_data) {
        ...
        // Render table
        do_action('wishglut_wishlist_pro_multilist_tabs', 10, 'multilist-tabs');
    ...
    public function render_wishlist_action_buttons($product_ids_filter, $wishlist_type = 'main', $list_name = '') {
        do_action( 'wishglut_before_render_wishlist_action_buttons', $product_ids_filter, $wishlist_type, $list_name );
        
        $this->internal_render_wishlist_action_buttons($product_ids_filter, $wishlist_type, $list_name);
        
        do_action( 'wishglut_after_render_wishlist_action_buttons', $product_ids_filter, $wishlist_type, $list_name );
    }
    
    public static function trigger_wishlist_action_buttons_render($product_ids_filter) {
        do_action( 'wishglut_render_wishlist_action_buttons', $product_ids_filter );
    }
    ...
    private function internal_render_wishlist_action_buttons($product_ids_filter, $wishlist_type = 'main', $list_name = '') {
        ...
        <div class="wishlist-action-buttons">
            ...
            if ( is_user_logged_in() && 
                ...
                do_action( 'wishglut_wishlist_subscribe_notifications', $product_ids_filter, $wishlist_type, $list_name );
           ...
           if (($this->enhancements['wishlist-enable-share-qr'] ) && 
             ...
             if (!defined('WISHGLUT_WISHLIST_PRO')) {
            ...
            <div class="wishlist-print">
                <button class="btn-print-wishlist" id="print-wishlist">
                    <?php echo esc_html__('Print Wishlist', 'wishglut'); ?>
                </button>
            </div>
            <?php } }
            if (!empty($product_ids_filter)) {
                do_action( 'wishglut_wishlist_all_products_print', $product_ids_filter );
            }
            ?>
        </div>
        <?php
    ...
    public function init_wishlist_action_buttons_hooks() {
        add_action( 'wishglut_render_wishlist_action_buttons', array( $this, 'handle_render_wishlist_action_buttons' ), 10, 3 );
    }

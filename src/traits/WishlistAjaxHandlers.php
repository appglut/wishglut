...
trait WishlistAjaxHandlers {
    ...
    public function wishglut_bulk_add_to_cart() {
        ...
        wp_send_json_success([
            ...
            'message' => sprintf(__('%d products added to cart', 'wishglut'), count($added_products))
        ]);
    }

    public function wishglut_add_all_to_cart() {
        check_ajax_referer( 'shopLayouts_nonce', 'nonce' );
    ...
    public function wishglut_remove_from_wishlist() {
        ...
        if ( $user_id && $product_id ) {
            ...
            if ( $wishlist_data ) {
                if ( $wishlist_type === 'main' ) {
                    ...
                    do_action( 'wishglut_remove_from_sublist', $product_id, $list_name, $user_id );
                }
...
// phpcs:ignore WordPress.DB.DirectDatabaseQuery.DirectQuery, WordPress.DB.DirectDatabaseQuery.NoCaching
    ...
    public function wishglut_bulk_remove_from_wishlist() {
        check_ajax_referer( 'shopLayouts_nonce', 'nonce' );
        ...
        wp_send_json_success([
            ...
            'message' => sprintf(__('%d products removed from wishlist', 'wishglut'), count($removed_products))
        ]);
    }
}
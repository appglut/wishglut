/**
 * Wishglut Wishlist - Free Version JavaScript
...
jQuery(document).ready(function ($) {
  ...
  function getOrCreateGuestId() {
    ...
    const guestIdCookie = document.cookie.split("; ").find((row) => row.startsWith("wishglutw_guest_user_id="));
  ...
  $("body").on("click", ".wishglut-wishlist-table .add-to-cart-btn", function () {
    ...
    $.ajax({
      ...
      data: {
        action: "wishglut_wishlist_add_to_cart",
  ...
  $("body").on("click", ".wishglut-wishlist-table .remove-btn", function () {
    ...
    $.ajax({
      ...
      url: ajax_data.ajax_url,
      data: {
        action: "wishglut_remove_from_wishlist",
 ...
 function loadWishlistContent() {
    $.ajax({
      ...
      url: ajax_data.ajax_url,
      data: {
        action: "wishglut_load_wishlist_content",
        nonce: ajax_data.nonce,
      },
      success: function (response) {
        if (response.success) {
  ...
  function showNotification(message) {
    const $notification = $("#wishglut-wishlist-notification");

    if ($notification.length === 0) {
      // Create notification div if it doesn't exist
      $('body').append('<div id="wishglut-wishlist-notification" style="display: none;"></div>');
    }

    // Clear any existing content and show notification
    $("#wishglut-wishlist-notification")
  ...
  function mergeGuestWishlist() {
    ...
    if (guestId && ajax_data.should_merge_wishlist) {
      $.ajax({
        ...
        type: "POST",
        data: {
          action: "wishglut_merge_guest_wishlist",
          guest_id: guestId,
          nonce: ajax_data.nonce,
        },
        success: function (response) {
  ...
  $(document).on(
    "click",
    ".wishglut_wishlist .not-shopgw-added, .wishglut_wishlist .shopgw-added , .wishglut_wishlist .already-added",
    function (e) {
      const $button = $(this);
      ...
      $.ajax({
        ...
        type: "POST",
        data: {
          action: "wishglut_toggle_wishlist",
  ...
  $(document).on("click", ".wishglut-wishlist-table .checkout-link, .wishglut_wishlist .checkout-link", function (e) {
    e.preventDefault();
    ...
    $.ajax({
      ...
      type: "POST",
      data: {
        action: "wishglut_add_to_cart_and_checkout",
        product_id: productID,
        quantity: quantity,
      },
      success: function (response) {

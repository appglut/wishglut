jQuery(document).ready(function($) {
    ...
    $('#wishglut-apply-action').on('click', function(e) {
        e.preventDefault();
        
        const selectedAction = $('#wishglut_product_actions').val();
    ...
    function findActiveWishlistContainer(element) {
        ...
        const singleContainer = element.closest('.wishglut-wishlist-table-container').parent();
    ...
    function sortWishlistTable(sortBy, container) {
        const table = container.find('.wishglut-wishlist-table');
        ...
        const sortedRows = productRows.toArray().sort(function(a, b) {
            ...
            switch(sortBy) {
                case 'name':
                    ...
                    if (!nameA) {
                        nameA = rowA.find('a.wishglut-product-link').text().trim();
                    }
                    if (!nameA) {
                    ...
                    if (!nameB) {
                        nameB = rowB.find('a.wishglut-product-link').text().trim();
                    }
                    if (!nameB) {
    ...
    function updateBulkActionButtons() {
        ...
        $('#wishglut-apply-action').prop('disabled', checkedCount === 0);
    }
    ...
    function bulkAddToCart(productIds, quantities) {
        $.ajax({
            url: wishlist_control_wishglut_ajax.ajax_url,
            type: 'POST',
            data: {
                action: 'wishglut_bulk_add_to_cart',
                nonce: wishlist_control_wishglut_ajax.nonce,
                product_ids: productIds,
                quantities: quantities
            },
            beforeSend: function() {
                $('#add-selected-to-cart').text('Adding...').prop('disabled', true);
                $('#wishglut-apply-action').text('Adding...').prop('disabled', true);
            },
            ...
            complete: function() {
                ...
                $('#wishglut-apply-action').text('Apply Action').prop('disabled', false);
            }
        });
    }
    ...
    function bulkRemoveFromWishlist(productIds) {
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'wishglut_bulk_remove_from_wishlist',
                nonce: wishlist_control_wishglut_ajax.nonce,
                product_ids: productIds
            },
            beforeSend: function() {
                $('#wishglut-apply-action').text('Removing...').prop('disabled', true);
            },
            success: function(response) {
                if (response.success) {
                    alert(response.data.message);
    ...
    function addAllToCart() {
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'wishglut_add_all_to_cart',
                nonce: wishlist_control_wishglut_ajax.nonce
            },
            beforeSend: function() {
    ...
    function printWishlist() {
        ...
        const wishlistContent = $('.wishglut-wishlist-table').clone();

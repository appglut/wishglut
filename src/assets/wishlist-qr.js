/**
 * Wishglut Wishlist QR Code Functionality - Simplified Version
 */
...
(function($) {
    ...
    function initQRFeatures() {
        ...
        $(document).on('click', '.wishglut-qr-share-btn', function(e) {
            e.preventDefault();
        ...
        $(document).on('click', '.wishglut-modal-close', function(e) {
            e.preventDefault();
            closeQRModal();
        });
        
        // Click outside modal to close
        $(document).on('click', '.wishglut-modal', function(e) {
            if (e.target === this) {
                closeQRModal();
            }
        });
    ...
    function generateQRCode(listName = '') {
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'wishglut_generate_qr_code',
    ...
    function generateClientSideQR(listName = '') {
        ...
        const shareUrl = baseUrl + '?view_shared=1&data=' + btoa(JSON.stringify({
            user_id: wishglut_ajax.user_id || 'guest',
            list_name: listName,
            timestamp: Date.now()
        }));
    ...
    function showQRModal() {
        $('#wishglut-qr-modal').fadeIn(300);
        $('body').addClass('wishglut-modal-open');
    }
    
    function closeQRModal() {
        $('#wishglut-qr-modal').fadeOut(300);
        $('body').removeClass('wishglut-modal-open');
    }
    ...
    function addSharedWishlistNotice() {
        ...
        $('.wishglut-shared-wishlist').prepend(notice);
    }
    ...
    window.createMyOwnWishlist = function() {
        ...
        const wishlistUrl = wishglut_ajax.wishlist_url || '/wishlist/';
    ...
    window.autoGenerateWishlistQR = function() {
        if ($('.wishglut-shared-wishlist').length > 0) {

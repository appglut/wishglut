 ...
 *
 */
if ( ! class_exists( 'AGSHOPGLUTP' ) ) {
	abstract class AGSHOPGLUTP extends AGSHOPGLUT_Abstract {
		...
		public function field_before() {
			return ( ! empty( $this->field['before'] ) ) ? '<div class="agl-before-text">' . $this->field['before'] . '</div>' : '';
		}

		public function field_after() {

			$output = ( ! empty( $this->field['after'] ) ) ? '<div class="agl-after-text">' . $this->field['after'] . '</div>' : '';
			$output .= ( ! empty( $this->field['desc'] ) ) ? '<div class="clear"></div><div class="agl-desc-text">' . $this->field['desc'] . '</div>' : '';
			$output .= ( ! empty( $this->field['help'] ) ) ? '<div class="agl-help"><span class="agl-help-text">' . $this->field['help'] . '</span><i class="fas fa-question-circle"></i></div>' : '';
			$output .= ( ! empty( $this->field['_error'] ) ) ? '<div class="agl-error-text">' . $this->field['_error'] . '</div>' : '';
		...
		public static function field_data( $type = '', $term = false, $query_args = array() ) {
			...
			switch ( $type ) {
				...
				case taxonomy_exists( $type ):
					...
					} else if ( array_key_exists( 'post_type', $query_args ) ) {
						if ( $query_args['post_type'] == 'all' ) {
							$query = get_terms( array( 'taxonomy' => $type, 'hide_empty' => false ) );
						} else {
							$query = get_terms( array( 'post_types' => $query_args['post_type'], 'taxonomy' => $type, 'hide_empty' => false ) );
					...
					if ( ! is_wp_error( $query ) && ! empty( $query->terms ) && ! in_array( 'product_type', $query_args ) ) {
					...
					if ( ! is_wp_error( $query ) && ! empty( $query ) && array_key_exists( 'post_type', $query_args ) ) {

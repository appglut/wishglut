 ...
 *
 */
if ( ! class_exists( 'AGWISHGLUT_Layouts' ) ) {
	class AGWISHGLUT_Layouts extends AGWISHGLUT_Abstract {
		...
		public function get_meta_value( $field ) {
			...
			$post_id = ! wp_verify_nonce( isset( $_POST['meta_nonce_check'] ), 'meta_nonce_value' ) && isset( $_GET['layout_id'] ) ? absint( $_GET['layout_id'] ) : 1;
			...
			foreach ( $this->sections as $section ) {
				...
				if ( ! empty( $section['fields'] ) ) {
					...
					foreach ( $section['fields'] as $field ) {
						...
						AGWISHGLUT::field( $field, $this->get_meta_value( $field ), $this->unique, 'metabox' );

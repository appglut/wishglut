	...
	class AGWISHGLUT {
		...
		public static function init() {
			...
			self::textdomain();
			 ...
			 add_action( 'after_setup_theme', array( 'AGWISHGLUT', 'setup' ) );
			 add_action( 'init', array( 'AGWISHGLUT', 'setup' ) );
			 add_action( 'switch_theme', array( 'AGWISHGLUT', 'setup' ) );
			 add_action( 'admin_enqueue_scripts', array( 'AGWISHGLUT', 'add_admin_enqueue_scripts' ) );
			add_action( 'wp_enqueue_scripts', array( 'AGWISHGLUT', 'add_typography_enqueue_styles' ), 80 );
			add_action( 'wp_head', array( 'AGWISHGLUT', 'add_custom_css' ), 80 );
			add_filter( 'admin_body_class', array( 'AGWISHGLUT', 'add_admin_body_class' ) );
		...
		public static function setup() {
			...
			if ( ! empty( self::$args['admin_options'] ) ) {
				foreach ( self::$args['admin_options'] as $key => $value ) {
					if ( ! empty( self::$args['sections'][ $key ] ) && ! isset( self::$inited[ $key ] ) ) {
						...
						AGWISHGLUT_Options::instance( $key, $params );
			...
			$params = array();
			if ( class_exists( 'AGWISHGLUT_Tax_Options' ) && ! empty( self::$args['taxonomy_options'] ) ) {
				$taxonomy = '';
				foreach ( self::$args['taxonomy_options'] as $key => $value ) {
					if ( ! empty( self::$args['sections'][ $key ] ) && ! isset( self::$inited[ $key ] ) ) {
						...
						AGWISHGLUT_Tax_Options::instance( $key, $params );
			...
			if ( ! empty( self::$args['metabox_options'] ) ) {
				foreach ( self::$args['metabox_options'] as $key => $value ) {
					if ( ! empty( self::$args['sections'][ $key ] ) && ! isset( self::$inited[ $key ] ) ) {
						...
						AGWISHGLUT_Metabox::instance( $key, $params );
			...
			if ( ! empty( self::$args['layout_options'] ) ) {
				foreach ( self::$args['layout_options'] as $key => $value ) {
					if ( ! empty( self::$args['sections'][ $key ] ) && ! isset( self::$inited[ $key ] ) ) {
						...
						AGWISHGLUT_Layouts::instance( $key, $params );
			...
			$params = array();
			if ( ! empty( self::$args['ecustomizer_options'] ) ) {
				foreach ( self::$args['ecustomizer_options'] as $key => $value ) {
					if ( ! empty( self::$args['sections'][ $key ] ) && ! isset( self::$inited[ $key ] ) ) {
						...
						AGWISHGLUT_eCustomizer::instance( $key, $params );
		...
		public static function createEcustomSettings( $id, $args = array() ) {
			self::$args['ecustomizer_options'][ $id ] = $args;
		}
		...
		public static function includes() {
			...
			//self::include_plugin_file( 'classes/ecustomizer.class.php' );

		}

		// Maybe include a field class
		public static function maybe_include_field( $type = '' ) {
			if ( ! class_exists( 'AGWISHGLUT_' . $type ) && class_exists( 'AGWISHGLUTP' ) ) {
		...
		public static function add_admin_enqueue_scripts() {
			...
			if ( ! empty( self::$fields ) ) {
				foreach ( self::$fields as $field ) {
					if ( ! empty( $field['type'] ) ) {
						$classname = 'AGWISHGLUT_' . $field['type'];
		...
		public static function field( $field = array(), $value = '', $unique = '', $where = '', $parent = '' ) {
			...
			if ( ! empty( $field_type ) ) {
				...
				$classname = 'AGWISHGLUT_' . $field_type;

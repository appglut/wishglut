 ...
 *
 */
if ( ! class_exists( 'AGWISHGLUT_select_options' ) ) {
  class AGWISHGLUT_select_options extends AGWISHGLUTP {
      ...
      if ( isset( $this->field['options'] ) ) {
        ...
        if ( ( is_array( $options ) && ! empty( $options ) ) || ( ! empty( $args['chosen'] ) && ! empty( $args['ajax'] ) ) ) {
          ...
          if ( ! empty( $args['chosen'] ) && ! empty( $args['multiple'] ) ) {
            ...
            echo '<select name="'. esc_attr($field_name) .'" class="agl-hide-select hidden"'. esc_attr($multiple_attr . $field_attr) .'>';

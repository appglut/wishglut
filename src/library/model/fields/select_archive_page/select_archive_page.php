...
/**
 *
 * Field: select_archive_page
 *
 * @since 1.0.0
 * @version 1.0.0
 *
 */
if ( ! class_exists( 'AGSHOPGLUT_select_archive_page' ) ) {
	class AGSHOPGLUT_select_archive_page extends AGSHOPGLUTP {
		...
		public function render() {
			...
			if ( isset( $this->field['options'] ) ) {
				...
				if ( is_string( $maybe_options ) && ! empty( $args['chosen'] ) && ! empty( $args['ajax'] ) ) {
					...
					if ( ! empty( $this->value ) && is_array( $this->value ) ) {
						foreach ( $this->value as $value ) {
							...
							if ( $value === 'Woo Shop Page' || $value === 'All Categories' || $value === 'All Tags' ) {
								$options[ $value ] = $value;
				...
				} else if ( is_string( $maybe_options ) && $maybe_options === 'select_archive_page' ) {
					...
					foreach ( $all_layouts as $layout ) {
						...
						// Fixed path to match actual database structure: shopg_options_settings → shopg_settings_options → shopg_advanced_settings_accordion → select-archive-pages
						if ( isset( $settings['shopg_options_settings']['shopg_settings_options']['shopg_advanced_settings_accordion']['select-archive-pages'] ) ) {
							$locations = $settings['shopg_options_settings']['shopg_settings_options']['shopg_advanced_settings_accordion']['select-archive-pages'];
				...
				foreach ( $options as $option_key => $option ) {
					...
					if ( is_array( $option ) && ! empty( $option ) ) {
						...
						foreach ( $option as $sub_key => $sub_value ) {
							...
							// Check if this option is used by another layout
							$disabled = ( isset( $this->used_options ) && in_array( $sub_key, $this->used_options ) ) ? ' disabled' : '';
							$disabled_text = $disabled ? ' (Used by another layout)' : '';

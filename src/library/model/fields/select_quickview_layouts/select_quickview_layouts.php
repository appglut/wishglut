...
/**
 *
 * Field: select_quickview_layouts
 * Custom field to select quick view layouts
 *
 */
if ( ! class_exists( 'AGSHOPGLUT_select_quickview_layouts' ) ) {
	class AGSHOPGLUT_select_quickview_layouts extends AGSHOPGLUTP {
		...
		public function render() {
			...
			// Get all quickview layouts from database
			global $wpdb;
			...
			$quickview_layouts = $wpdb->get_results(
				"SELECT id, layout_name FROM `" . esc_sql($table_name) . "` ORDER BY id DESC",
				ARRAY_A
			);

			$options = array();
			if ( ! empty( $quickview_layouts ) && is_array( $quickview_layouts ) ) {
				foreach ( $quickview_layouts as $layout ) {
			...
			if ( ! empty( $options ) ) {
				...
				echo '<option value="" disabled>' . esc_html__( 'No quick view layouts found', 'shopglut' ) . '</option>';
			}

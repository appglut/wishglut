 ...
 * @version 1.0.0
 */
if ( ! class_exists( 'AGWISHGLUT_dimensions' ) ) {
	class AGWISHGLUT_dimensions extends AGWISHGLUTP {
		...
		public function render() {
			...
			if ( $is_pro ) {
				...
				if ( ! empty( $args['width'] ) ) {
					...
					echo ( ! empty( $args['width_icon'] ) ) ? '<span class="agl--label agl--icon">' . wp_kses_post($args['width_icon']) . '</span>' : '';
					echo '<input type="number" name="' . esc_attr( $this->field_name( '[width]' ) ) . '" value="' . esc_attr( $value['width'] ) . '"' . wp_kses_post($placeholder) . ' class="agl-input-number' . esc_attr( $is_unit ) . '" step="any" />';
				...
				if ( ! empty( $args['height'] ) ) {
					...
					echo '<div class="agl--input">';
					echo ( ! empty( $args['height_icon'] ) ) ? '<span class="agl--label agl--icon">' . wp_kses_post($args['height_icon']) . '</span>' : '';
					echo '<input type="number" name="' . esc_attr( $this->field_name( '[height]' ) ) . '" value="' . esc_attr( $value['height'] ) . '"' . wp_kses_post($placeholder). ' class="agl-input-number' . esc_attr( $is_unit ) . '" step="any" />';
					echo ( ! empty( $unit ) ) ? '<span class="agl--label agl--unit">' . esc_attr( $args['units'][0] ) . '</span>' : '';
					echo '</div>';
				}

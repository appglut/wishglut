...
/**
 *
 * Field: swatch_select_products
 *
 * Product selector field specifically for swatches to avoid conflicts
 * with other select_products instances in repeaters
 *
 * @since 1.0.0
 * @version 1.0.0
 *
 */
if ( ! class_exists( 'AGSHOPGLUT_swatch_select_products' ) ) {
	class AGSHOPGLUT_swatch_select_products extends AGSHOPGLUTP {
		...
		public function render() {
			...
			$unique_instance_id = 'swatch-product-select-' . uniqid();
			...
			$field_class = ( $args['chosen'] ) ? ' class="swatch-chosen-select swatch-product-selector' . esc_attr( $chosen_rtl . $chosen_sortable . $chosen_ajax ) . '"' : '';
			...
			if ( $product_query->have_posts() ) {
				while ( $product_query->have_posts() ) {
					...
					$options[ $product_id ] = $product_title . ' (#' . $product_id . ')';
				}
				wp_reset_postdata();
			}

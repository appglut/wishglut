...
if ( ! class_exists( 'AGWISHGLUT_taxonomy' ) ) {
  class AGWISHGLUT_taxonomy extends AGWISHGLUTP {
    ...
    public function render() {
      ...
      if ( isset( $this->field['options'] ) ) {
        ...
        if(empty($args['extra']) && empty($args['terms']))
        {
          $maybe_options    =  get_object_taxonomies(get_post_types(array('public'   => true ), 'names', 'and' ));
        }
        else if(empty($args['extra']) && !empty($args['terms']))
        {
          $maybe_options =  $args['terms'];
        } 
        else
        {
          $maybe_options    =  get_object_taxonomies($args['extra']);
        }
        ...
        if ( ( is_array( $options ) && ! empty( $options ) ) || ( ! empty( $args['chosen'] ) && ! empty( $args['ajax'] ) ) ) {
          ...
          echo '<select id="test" name="'. esc_attr( $field_name ) .'"'. wp_kses_post($field_class. $multiple_attr . $placeholder_attr . $field_attr . $chosen_data_attr) .'>';
          ...
          foreach ( $options as $option_key => $option ) {
            ...
            {
              $selected = ( in_array( $option, $this->value ) ) ? ' selected' : '';

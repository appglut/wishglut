<?php
if ( ! class_exists( 'AGWISHGLUT_tax_images' ) ) {
	class AGWISHGLUT_tax_images extends AGWISHGLUTP {
		...
		public function render() {
			...
			echo '<button type="button" class="button button-primary open-modal" data-modal-id="modal-' . esc_attr($unique_modal_id) . '">' . esc_html__( 'Select Image for Terms', 'wishglut' ) . '</button>';
			...
			echo '<h3>' . esc_html__( 'Add Image for Terms', 'wishglut' ) . '</h3>';
		...
		private function render_terms_with_media_buttons( $taxonomy ) {
			...
			if ( ! empty( $terms ) && ! is_wp_error( $terms ) ) {
				foreach ( $terms as $term ) {
					...
					echo '<button type="button" class="button add-image" data-term-id="' . esc_attr( $term_id ) . '" data-repeatable-id="' . esc_attr( $repeatable_id_string ) . '">' . esc_html__( 'Add Image', 'wishglut' ) . '</button>';
					...
					// Remove button
					echo '<button type="button" class="button remove-image" data-term-id="' . esc_attr( $term_id ) . '" data-repeatable-id="' . esc_attr( $repeatable_id_string ) . '" style="' . ( $image_url ? '' : 'display:none;' ) . '">' . esc_html__( 'Remove Image', 'wishglut' ) . '</button>';
			...
			echo '<input type="submit" id="save_filter_images" class="button button-primary" value="' . esc_html__( 'Save Images', 'wishglut' ) . '">';
			echo '</div>';
		}

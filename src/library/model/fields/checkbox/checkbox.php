 ...
 *
 */
if ( ! class_exists( 'AGSHOPGLUT_checkbox' ) ) {
	class AGSHOPGLUT_checkbox extends AGSHOPGLUTP {
		...
		public function render() {
			...
			$inline_class = ( $args['inline'] ) ? ' class="agl--inline-list"' : '';
			...
			if ( isset( $this->field['options'] ) && ( $this->field['options'] != 'all_terms' ) ) {
					...
					foreach ( $options as $option_key => $option_value ) {
						...
						if ( is_array( $option_value ) && ! empty( $option_value ) ) {
							...
							foreach ( $option_value as $sub_key => $sub_value ) {
								...
								echo '<span class="agl--text">' . esc_attr( $sub_value ) . '</span>';
								echo '</label>';
								echo '</li>';
							}
							echo '</ul>';
							...
							echo '<span class="agl--text">' . esc_attr( $option_value ) . '</span>';
							echo '</label>';
							echo '</li>';
				...
				echo '<label class="agl-checkbox">';
				echo '<input type="hidden" name="' . esc_attr( $this->field_name() ) . '" value="' . esc_attr( $this->value ) . '" class="agl--input"' . wp_kses_post( $this->field_attributes() ) . '/>';
				echo '<input type="checkbox" name="_pseudo" class="agl--checkbox"' . esc_attr( checked( $this->value, 1, false ) ) . '/>';
				echo ( ! empty( $this->field['label'] ) ) ? '<span class="agl--text">' . esc_attr( $this->field['label'] ) . '</span>' : '';
				echo '</label>';

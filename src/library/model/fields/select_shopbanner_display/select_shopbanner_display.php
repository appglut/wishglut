...
/**
 *
 * Field: select_shopbanner_display
 *
 * @since 1.0.0
 * @version 1.0.0
 *
 */
if ( ! class_exists( 'AGSHOPGLUT_select_shopbanner_display' ) ) {
	class AGSHOPGLUT_select_shopbanner_display extends AGSHOPGLUTP {
		...
		public function render() {
			...
			if ( isset( $this->field['options'] ) ) {
				...
				if ( is_string( $maybe_options ) && $maybe_options === 'select_shopbanner_display' ) {
					...
					if ( $pages_query->have_posts() ) {
						while ( $pages_query->have_posts() ) {
							...
							$specific_pages[get_the_ID()] = get_the_title();
						}
						wp_reset_postdata();
					}

					if ( ! empty( $specific_pages ) ) {
						$options['Specific Pages'] = $specific_pages;
					}

					// Products Group (if WooCommerce is active)
					if ( class_exists( 'WooCommerce' ) ) {
						...
						if ( $products_query->have_posts() ) {
							while ( $products_query->have_posts() ) {
								...
								$specific_products[get_the_ID()] = get_the_title();
							}
							wp_reset_postdata();
						}

						if ( ! empty( $specific_products ) ) {
							$options['Specific Products'] = $specific_products;
						}
					}

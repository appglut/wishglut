...
/**
 *
 * Field: select_quickview_display
 *
 * @since 1.0.0
 * @version 1.0.0
 *
 */
if ( ! class_exists( 'AGWISHGLUT_select_quickview_display' ) ) {
	class AGWISHGLUT_select_quickview_display extends AGWISHGLUTP {
		...
		public function render() {
			...
			if ( isset( $this->field['options'] ) ) {
				...
				if ( is_string( $maybe_options ) && ! empty( $args['chosen'] ) && ! empty( $args['ajax'] ) ) {
				...
				} else if ( is_string( $maybe_options ) && $maybe_options === 'select_quickview_display' ) {
					...
					$all_layouts = $wpdb->get_results( "SELECT id, layout_settings FROM `{$wpdb->prefix}wishglut_quickview_layouts`" );

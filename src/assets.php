...
class WishlistAssets {
    ...
    public function enqueue_frontend_assets() {
        ...
        if (file_exists(__DIR__ . '/assets/wishlist-counter.js')) {
            ...
            wp_localize_script('wishglut-wishlist-counter-js', 'wishglut_wishlist_ajax', array(
    ...
    private function add_dynamic_styles() {
        if (class_exists('\Wishglut\dynamicStyle')) {
            $wishlist_dynamic_style = new \Wishglut\dynamicStyle();
            $wishlist_dynamic_css = $wishlist_dynamic_style->dynamicCss();
            if (!empty($wishlist_dynamic_css)) {
                wp_add_inline_style('wishglut-wishlist-style', $wishlist_dynamic_css);
            }
        }
    }
    ...
    private function localize_wishlist_script() {
        $wishlist_options = get_option('agwishglut_wishlist_options', array());
        
        wp_localize_script('wishglut-wishlist-js', 'ajax_data', array(
            'ajax_url' => admin_url('admin-ajax.php'),
            'nonce' => wp_create_nonce('wishglut_wishlist_nonce'),

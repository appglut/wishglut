 ...
 * This ensures the individual menu shows the same content as Wishglut's wishlist submenu
 */
class WishglutIndividualMenu {
	...
	public function showIndividualMenu() {
		...
		if ( isset( $settings['wishglut-show-menu'] ) ) {
			// AGWISHGLUT framework stores '1' or '0' as string, check for both
	...
	/**
	 * Render the page - loads the same WishlistMenuHandler that Wishglut uses
	...
	public function renderPage() {
		...
		if ( file_exists( WP_PLUGIN_DIR . '/wishglut/src/WishlistMenuHandler.php' ) ) {
			...
			$wishlist_handler = new \Wishglut\WishlistMenuHandler('wishglut');
	...
	public function enqueueAssets() {
		// Enqueue the same assets that Wishglut uses for wishlist page
		do_action( 'load-wishglut_page_wishglut_wishlist' );
	}

...
if ( ! class_exists( 'dataManage' ) ) {
	...
	class dataManage {
		...
		public function ajax_get_all_activity() {
			check_ajax_referer( 'wishglut_dashboard_nonce', 'nonce' );
			...
			<table class="dashboard-table">
				<thead>
					<tr>
						<th><?php esc_html_e( 'User', 'wishglut' ); ?></th>
						<th><?php esc_html_e( 'Type', 'wishglut' ); ?></th>
						<th><?php esc_html_e( 'Product', 'wishglut' ); ?></th>
						<th><?php esc_html_e( 'Action', 'wishglut' ); ?></th>
				...
				<tbody>
					...
					if ( ! empty( $activities ) ) {
						foreach ( $activities as $activity ) {
							...
							foreach ( $product_ids as $product_id ) {
								...
								<tr>
									...
									<td><span class="user-type-badge <?php echo esc_attr( $activity->user_type ); ?>"><?php echo 'guest' === $activity->user_type ? esc_html__( 'Guest', 'wishglut' ) : esc_html__( 'Registered', 'wishglut' ); ?></span></td>
									...
									<td><span class="action-badge added"><?php esc_html_e( 'Added to Wishlist', 'wishglut' ); ?></span></td>
						...
						?>
						<tr>
							<td colspan="5"><?php esc_html_e( 'No activity found.', 'wishglut' ); ?></td>
						</tr>
						<?php
					}
					?>
		...
		public function ajax_export_top_products() {
			...
			$filename = 'wishglut-top-products-' . wp_date( 'Y-m-d' ) . '.csv';
		...
		public function ajax_export_all_data() {
			...
			try {
				...
				$filename = 'wishglut-wishlist-export-' . wp_date( 'Y-m-d-H-i-s' ) . '.csv';
		...
		public function ajax_export_guest_data() {
			...
			$filename = 'wishglut-guest-data-' . wp_date( 'Y-m-d' ) . '.csv';
		...
		public function ajax_export_registered_data() {
			...
			$filename = 'wishglut-registered-data-' . wp_date( 'Y-m-d' ) . '.csv';
				...
				wp_send_json_success(
					array(
						...
						'message'       => sprintf(
							...
							esc_html__( 'Deleted %1$d old wishlist entries older than %2$d days.', 'wishglut' ),
							$deleted_count,
							$days_old
						),
					)
...
// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			...
			<div class="field-mapping-container">
				...
				<div class="mapping-row">
					...
					<select data-field="<?php echo esc_attr( $db_field ); ?>">
						<option value="">-- <?php esc_html_e( 'Select Column', 'wishglut' ); ?> --</option>
						<?php foreach ( $headers as $header ) : ?>
							<?php
			...
			$_SESSION['wishglut_import_data'] = array(
				'headers' => $headers,
		...
		public function ajax_import_wishlist_data() {
			...
			if ( empty( $_SESSION['wishglut_import_data'] ) ) {
				wp_send_json_error( array( 'message' => 'No import data found. Please upload file again.' ) );
			}

			// phpcs:ignore WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			$import_data         = isset( $_SESSION['wishglut_import_data'] ) ? (array) $_SESSION['wishglut_import_data'] : array();
			...
			try {
				...
				unset( $_SESSION['wishglut_import_data'] );
				...
				wp_send_json_success(
					array(
						...
						'message'        => sprintf(
							...
							esc_html__( 'Successfully imported %d records.', 'wishglut' ),
							$imported_count
						),
					)
				);

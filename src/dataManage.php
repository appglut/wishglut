...
class dataManage {
    ...
    public function __construct() {
        ...
        $this->enhancements = get_option( 'agwishglut_wishlist_options' );
        $this->cron_token = get_option( 'wishglut_wishlist_cron_token', '' );
        $this->wishglut_schedule_guest_cleanup();


        if ( empty( $this->cron_token ) ) {
            $this->cron_token = $this->generate_cron_token();
            update_option( 'wishglut_wishlist_cron_token', $this->cron_token );
        }
        $this->add_actions();
    ...
    private function add_actions() {
        ...
        add_action( 'init', function () {
            if ( ! is_user_logged_in() && ! isset( $_COOKIE['wishglutw_guest_user_id'] ) ) {
                $guest_id = 'guest_' . uniqid();
                setcookie( 'wishglutw_guest_user_id', $guest_id, time() + ( 86400 * 30 ), '/' ); // Cookie expires in 30 days
            }
        } );

        // Register shortcode
        add_shortcode( 'wishglut_wishlist', [ $this, 'wishglut_wishlist_shortcode' ] );

        // Register AJAX actions
        add_action( 'wp_ajax_wishglut_load_wishlist_content', [ $this, 'wishglut_load_wishlist_content' ] );
        add_action( 'wp_ajax_nopriv_wishglut_load_wishlist_content', [ $this, 'wishglut_load_wishlist_content' ] );

        add_action( 'wp_ajax_wishglut_load_account_wishlist_content', [ $this, 'wishglut_load_account_wishlist_content' ] );
        add_action( 'wp_ajax_nopriv_wishglut_load_account_wishlist_content', [ $this, 'wishglut_load_account_wishlist_content' ] );

        add_action( 'wp_ajax_wishglut_remove_from_wishlist', [ $this, 'wishglut_remove_from_wishlist' ] );
        add_action( 'wp_ajax_nopriv_wishglut_remove_from_wishlist', [ $this, 'wishglut_remove_from_wishlist' ] );

        add_action( 'wp_ajax_wishglut_wishlist_add_to_cart', [ $this, 'wishglut_wishlist_add_to_cart' ] );
        add_action( 'wp_ajax_nopriv_wishglut_wishlist_add_to_cart', [ $this, 'wishglut_wishlist_add_to_cart' ] );

        add_action( 'wp_ajax_wishglut_add_to_cart_and_checkout', [ $this, 'wishglut_add_to_cart_and_checkout' ] );
        add_action( 'wp_ajax_nopriv_wishglut_add_to_cart_and_checkout', [ $this, 'wishglut_add_to_cart_and_checkout' ] );


        // AJAX handlers for wishlist actions
        add_action( 'wp_ajax_wishglut_toggle_wishlist', [ $this, 'wishglut_toggle_wishlist_callback' ] );
        add_action( 'wp_ajax_nopriv_wishglut_toggle_wishlist', [ $this, 'wishglut_toggle_wishlist_callback' ] );
        add_action( 'wp_ajax_wishglut_merge_guest_wishlist', [ $this, 'wishglut_merge_guest_wishlist' ] );
        add_action( 'wp_ajax_nopriv_wishglut_merge_guest_wishlist', [ $this, 'wishglut_merge_guest_wishlist' ] );


        add_action('wp_ajax_wishglut_add_all_to_cart', array($this, 'wishglut_add_all_to_cart'));
        add_action('wp_ajax_nopriv_wishglut_add_all_to_cart', array($this, 'wishglut_add_all_to_cart'));


        add_action('wp_ajax_wishglut_bulk_add_to_cart', array($this, 'wishglut_bulk_add_to_cart'));
        add_action('wp_ajax_nopriv_wishglut_bulk_add_to_cart', array($this, 'wishglut_bulk_add_to_cart'));
        
        add_action('wp_ajax_wishglut_bulk_remove_from_wishlist', array($this, 'wishglut_bulk_remove_from_wishlist'));
        add_action('wp_ajax_nopriv_wishglut_bulk_remove_from_wishlist', array($this, 'wishglut_bulk_remove_from_wishlist'));

          // QR Code functionality
        add_action('wp_ajax_wishglut_generate_qr_code', array($this, 'wishglut_generate_qr_code'));
        add_action('wp_ajax_nopriv_wishglut_generate_qr_code', array($this, 'wishglut_generate_qr_code'));


        // Set a transient when a user logs in to potentially merge guest wishlist data
        add_action( 'wp_login', [ $this, 'wishglut_set_merge_wishlist_transient' ], 10, 2 );
        add_action('wishglut_daily_guest_cleanup', array($this, 'wishglut_cleanup_old_guest_products'));



        // Register wishlist buttons AFTER WooCommerce is fully initialized
        add_action( 'wp', [ $this, 'wishglut_register_wishlist_buttons' ] );
        ...
        if ( isset( $this->enhancements['wishlist-page-account-page'] ) && 
             ...
             (!defined('WISHGLUT_WISHLIST_PRO') || (defined('WISHGLUT_WISHLIST_PRO') && WISHGLUT_WISHLIST_PRO !== true)) ) {
        ...
        if (!defined('WISHGLUT_WISHLIST_PRO') || (defined('WISHGLUT_WISHLIST_PRO') && WISHGLUT_WISHLIST_PRO !== true)) {
                // Register shortcode for wishlist count
                add_shortcode('wishglut_wishlist_count', array($this, 'wishlist_count_shortcode'));
